...
- **Global scope**: `C:/GitProject/CH32V303CBT/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/GitProject/CH32V303CBT/.claude/notes/`)
...
2. Glob `C:/GitProject/CH32V303CBT/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/GitProject/CH32V303CBT/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/GitProject/CH32V303CBT/.claude/notes/` — works in any directory
