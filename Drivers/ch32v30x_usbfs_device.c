...
/* Line Coding storage for CDC ACM — holds last SET_LINE_CODING data, used for GET_LINE_CODING response */
/* Default: 115200 baud, 1 stop bit, no parity, 8 data bits */
static uint8_t LineCoding[7] = {
    0x00, 0xC2, 0x01, 0x00,   /* 115200 = 0x0001C200 little-endian */
    0x00,                       /* 0 = 1 stop bit */
    0x00,                       /* 0 = no parity */
    0x08                        /* 8 data bits */
};
                    ...
                    case USBFS_UIS_TOKEN_OUT | DEF_UEP0:
                        ...
                        if ( intst & USBFS_UIS_TOG_OK )
                        {
                            if ( ( USBFS_SetupReqType & USB_REQ_TYP_MASK ) != USB_REQ_TYP_STANDARD )
                            {
                                ...
                                {
                                      /* Save line coding parameters for CDC ACM compliance.
                                         The downlinked data is 7 bytes in order:
                                         4 bytes: baud rate (little-endian)
                                         1 byte:  stop bits (0=1, 1=1.5, 2=2)
                                         1 byte:  parity (0=None, 1=Odd, 2=Even, 3=Mark, 4=Space)
                                         1 byte:  data bits (5,6,7,8,16)
                                         Parameters are stored for GET_LINE_CODING response,
                                         no UART reconfiguration is performed. */
                                      LineCoding[ 0 ] = USBFS_EP0_Buf[ 0 ];
                                      LineCoding[ 1 ] = USBFS_EP0_Buf[ 1 ];
                                      LineCoding[ 2 ] = USBFS_EP0_Buf[ 2 ];
                                      LineCoding[ 3 ] = USBFS_EP0_Buf[ 3 ];
                                      LineCoding[ 4 ] = USBFS_EP0_Buf[ 4 ];
                                      LineCoding[ 5 ] = USBFS_EP0_Buf[ 5 ];
                                      LineCoding[ 6 ] = USBFS_EP0_Buf[ 6 ];
                                 }
                            }
                            else
                            {
                    ...
                    if( USBFS_SetupReqType & USB_REQ_TYP_CLASS )
                    {
                        ...
                        switch( USBFS_SetupReqCode )
                        {
                            case CDC_GET_LINE_CODING:
                                pUSBFS_Descr = (uint8_t *)&LineCoding[ 0 ];
                                len = 7;
                                break;
